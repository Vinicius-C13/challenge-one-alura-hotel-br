...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtDataE.getDateEditor().addPropertyChangeListener(listener);
		panel.add(txtDataE);
		...
		txtDataS.addPropertyChangeListener(listener);
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
					if (txtValor.getText().length() < 1) {
						JOptionPane.showMessageDialog(null, "Verifique se as datas seleciondas são válidas.");
					} else {
						double valor = calcularValorEstadia();
						String dataE = new SimpleDateFormat("yyyy-MM-dd").format(txtDataE.getDate());
						String dataS = new SimpleDateFormat("yyyy-MM-dd").format(txtDataS.getDate());
	...
	public double calcularValorEstadia() {
		long diffInMillies = txtDataS.getDate().getTime() - txtDataE.getDate().getTime();
